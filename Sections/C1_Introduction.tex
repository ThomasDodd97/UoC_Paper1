...
Of these renewables, wind power saw the greatest growth in capacity to
...
Wind turbine blades are commonly manufactured from a composite 
 ...
 strong thermoset matrices are chemically crosslinked which makes them
...
The motivation for the present work was to ascertain whether Bayesian optimisation
 could be used to sample efficiently improve the extent of polymerisation
 in a novel biodegradable bio-based thermoset.

The composite matrix used for wind turbine blades is comprised of a moulding 
 thermoset, with the most commonly manufactured pre-thermoset form being 
...
These GP-UPRs are based on a mixture of a reactive diluent (e.g. styrene) and 
 a linear aromatic unsaturated polyester (e.g. poly(propylene maleate
...
An archetypal example of an aliphatic polyester would be poly(glycerol
...
In the present work we aim to avoid these feedstocks and focussed instead
 upon a bio-derived alternative, with inspiration drawn from a chemical 
...
Given this, we identified mass loss as a variable of interest during the
 ...
 extent of polymerisation, and sought to optimise this by 
 tuning the stoichiometries of the prepolymer resins mixed.
Extent of polymerisation could be an indicator of the crosslink density of 
...
\textcolor{RawSienna}{The present work also details a standalone experiment 
 which links this mass loss proxy variable with tetrahydrofuran-insoluble
 fraction in cured thermoset samples.}
...
Additionally, reaction times for rigid solid thermoset materials are on the
 order of \textcolor{purple}{days} when uncatalysed 
 \autocite{Trenkel2008} (catalysis commonly
 involves strong fuming acids which can corrode 
 equipment \autocite{Shukla2022}).
...
Kriging is preferable to response surface modelling since it makes no
 assumption of polynomial order, avoiding a priori bias in the 
 surrogate models generated.
Furthermore, radial basis function-based surrogate models provide no
 ...
 associated uncertainties derived using kriging provide the means by which
...
Hereafter, we detail the algorithmic mechanism behind the Bayesian 
 ...
 work.
The methods section concludes with a brief overview of how the physical system
...
The paper concludes that the Bayesian sequential routines succeeded in
 ...
 limit.
\textcolor{RawSienna}{
Simulations further demonstrated statistically that the probability of
 improvement and expected improvement acquisition functions
 respectively exhibited probabilities of 23.8\% and 65.0\%, respectively, of
 discovering values approaching the emulated global optimum,
 far surpassing the various benchmark techniques.
}
 \sout{whilst also statistically proving that the `probability of 
